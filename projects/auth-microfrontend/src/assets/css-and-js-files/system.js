...
(function () {
  ...
  function resolveImportMap(importMap, resolvedOrPlain, parentUrl) {
    ...
    return (
      applyPackages(resolvedOrPlain, importMap.imports) || (resolvedOrPlain.indexOf(':') !== -1 && resolvedOrPlain)
    );
  }
  ...
  function processScripts() {
    [].forEach.call(document.querySelectorAll('script'), function (script) {
      ...
      if (script.type === 'systemjs-module') {
        ...
        System.import(
          script.src.slice(0, 7) === 'import:' ? script.src.slice(7) : resolveUrl(script.src, baseUrl),
        ).catch(function (e) {
  ...
  systemJSPrototype.instantiate = function (url, parent) {
    ...
    return this.fetch(url, {
      ...
      if (!res.ok)
        throw Error(
          errMsg(7, res.status + ' ' + res.statusText + ', loading ' + url + (parent ? ' from ' + parent : '')),
        );
      var contentType = res.headers.get('content-type');
      if (!contentType || !jsContentTypeRegEx.test(contentType))
        throw Error(
          errMsg(4, 'Unknown Content-Type "' + contentType + '", loading ' + url + (parent ? ' from ' + parent : '')),
        );
  ...
  systemJSPrototype.set = function (id, module) {
    ...
    if (toStringTag$1 && module[toStringTag$1] === 'Module') {
      ...
      if (toStringTag$1) Object.defineProperty(ns, toStringTag$1, { value: 'Module' });
    }
